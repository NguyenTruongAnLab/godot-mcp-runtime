...
Set `GODOT_PATH` to your Godot 3.5+ executable for runtime tests and manual exercises.
...
### 1. MCP stdio transport - `console.log` is forbidden
...
### 2. Mutation operations auto-save
...
All handlers validate paths through `validateProjectArgs` or `validateSceneArgs` from `src/utils/godot-runner.ts`. `validateProjectArgs` rejects relative steps like `..` and verifies `project.godot` exists. `validateSceneArgs` additionally rejects absolute paths that escape the project root via `validateSubPath`. For scene-tree node paths (e.g. `root/Player`), use `validateNodePath` - it allows the relative-path style that `validateSubPath` rejects. Don't construct paths ad hoc with `path.join` - route through the validators so the security rules stay centralized.

### 4. Transactional Offline Snapshots

When implementing offline file modifications, the Node layer must register the file with the snapshot supervisor (`snapshot-manager.ts`). This automatically backs up target `.tscn` and `.tres` files to `.mcp/backups/` before execution. If validation checks or syntax tests fail on the output file, the manager automatically rolls the file back to its pristine backup copy, preventing scene corruptions.

### 5. TypeScript camelCase, GDScript snake_case
...
`run_project` spawns Godot without `-d` so runtime errors don't pause the engine and stall the McpBridge. The trade-off is that the `breakpoint` keyword in user code does nothing. There is no debugger attached. Use `print()` and `get_debug_output` instead.
