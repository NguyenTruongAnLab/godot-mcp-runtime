...
export const tilemapToolDefinitions: ToolDefinition[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        tileId: {
          type: 'integer',
          description: 'The tile ID to assign. Use -1 to clear the cell.',
        },
        ...
        transpose: {
          type: 'boolean',
          description: 'Whether to transpose the tile (rotate 90 deg and flip)',
        },
      },
  ...
  },
  {
    ...
    description:
      'Set a specific cell in a GridMap node (3D). Saves automatically. Can place an item by its ID or clear it by passing -1. Supports setting cell orientation. Returns a plain-text confirmation.',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: { type: 'string', description: 'Path to the Godot project directory' },
        ...
        orientation: {
          type: 'integer',
          description: 'Optional orientation index (0-23) for rotation',
        },
      },
      required: ['projectPath', 'scenePath', 'nodePath', 'x', 'y', 'z', 'item'],
    },
  },
];
...
export async function handleSetTilemapCell(runner: GodotRunner, args: OperationParams) {
  ...
  if (args.x === undefined || typeof args.x !== 'number') {
    return createErrorResponse('Valid x coordinate is required', [
      'Provide x coordinate as integer',
    ]);
  }
  if (args.y === undefined || typeof args.y !== 'number') {
    return createErrorResponse('Valid y coordinate is required', [
      'Provide y coordinate as integer',
    ]);
  }
...
export async function handleSetGridmapCell(runner: GodotRunner, args: OperationParams) {
  ...
  if (args.z === undefined || typeof args.z !== 'number') {
    return createErrorResponse('Valid z coordinate is required', [
      'Provide z coordinate as integer',
    ]);
  }
  if (args.item === undefined || typeof args.item !== 'number') {
    return createErrorResponse('Valid item ID is required', ['Provide item as integer']);
  }
